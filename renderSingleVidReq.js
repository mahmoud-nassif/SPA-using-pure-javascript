...
<div id="admin_video_res_container_${id}" class="input-group ml-2 mr-5 ${status!= 'done' ? 'd-none' : ''}">
  <input id="admin_video_res_${id}" type="text" class="form-control" placeholder="paste your youtube video here">
  <div class="input-group-append">
    <button id="admin_save_video_res_${id}" class="btn btn-outline-secondary" type="button">save</button>
  </div>
</div>
...
function bindAdminActions(id,state,status,videoRef,title){
  ...
  adminDeleteVideoReqElm.addEventListener('click',(e)=>{
    e.preventDefault()
    const isSure=confirm(`Are you sure to delete ${title}`)
    if(!isSure)return;
...
export function renderSingleVidReq(
  ...
  ,state,isPrepend=false){
    ...
    const vidReqElmContainer=document.createElement('div')
    vidReqElmContainer.innerHTML=`
    <div class="card mb-3">
    ...
    <div class="card-body d-flex justify-content-between flex-row">
      ...
      ${status==='done'?`<div class='ml-auto mr-3'><iframe width='240' height='135' src='https://www.youtube.com/embed/${videoRef.link}' frameborder='0' allowfullscreen></iframe></div>`:""}
    ...
    <div class="card-footer d-flex flex-row justify-content-between">
      <div class="${statusClass}">
        <span>${status.toUpperCase()} ${status==='done'?` on ${formatDate(videoRef.date)}`:''}</span>
    ...
    `
     ...
     isPrepend? listOfvidsElm.prepend(vidReqElmContainer): listOfvidsElm.appendChild(vidReqElmContainer)

     bindAdminActions(id,state,status,videoRef,title)



    //  applyVoteStyle(id,vidInfo.votes,state,status==='done')
    // applyVoteStyle(id,vidInfo.votes,status==='done',state)
    applyVoteStyle(id,votes,state.isSuperUser,state)
